...
library(ggplot2)
 ...
 write_csv(data_cleaned, "C:/Users/Polle/Downloads/cleaned_agricultural_data.csv")
 view(data_cleaned)
...
ggplot(data_cleaned, aes(x = Production_Quantity, y = Price_Per_Unit, color = Crop_Name)) +
 ...
 ggplot(harvest_summary, aes(x = Harvest_Season)) +
   ...
   geom_line(aes(y = Average_Price_Per_Unit * 10000, color = "Average Price Per Unit", group = 1), linewidth = 1) +  # Scale price for visibility
   scale_y_continuous(
